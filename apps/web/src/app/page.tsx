...
/**
 * @file page.tsx
 * @description Landing page facilitating instant ephemeral room creation,
 * room code search/join, and deep-link query parameter parsing.
 */
...
function LandingContent() {
  ...
  // Initialize ephemeral local user profile
  ...
  // Handler: Create Instant Ephemeral Room
  const handleCreateRoom = () => {
    const code = generateRoomCode();
    // Cache persona for room
    if (typeof window !== "undefined") {
  ...
  const handleJoinRoom = (e: React.FormEvent) => {
    ...
    if (!isValidRoomCode(clean)) {
      toast.error("Please enter a valid room code (e.g. SCRUM-492).");
      return;
    }
    if (typeof window !== "undefined") {
    ...
    <main className="flex-1 flex flex-col items-center justify-center p-4 sm:p-8 max-w-5xl mx-auto w-full">
      ...
      <div className="text-center max-w-2xl mx-auto mb-10">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-200/60 dark:border-indigo-800/60 text-indigo-700 dark:text-indigo-300 text-xs font-semibold mb-6">
          ...
          <span>100% Ephemeral &bull; Masked Blind Voting</span>
        </div>

        <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-slate-900 dark:text-slate-100 tracking-tight leading-none mb-4">
          Estimate Smarter. <br className="hidden sm:inline" />
          <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 via-indigo-400 to-purple-500">
            Zero Bias. Zero Bloat.
          </span>
        </h1>

        <p className="text-sm sm:text-base text-slate-600 dark:text-slate-400 max-w-xl mx-auto">
          Instant real-time planning poker rooms. Share a link, speak your story title with voice mic input, estimate with Fibonacci cards, and analyze consensus distributions without database tracking.
        </p>
      </div>
      ...
      <div className="mb-8 p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-md flex items-center gap-3">
        ...
        <button
          ...
          title="Customize Moniker"
        >
          <Edit2 className="w-3.5 h-3.5" />
        </button>
      </div>

      {/* Dual Entry Actions: Create or Join */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-2xl mb-12">
        ...
        <div className="p-6 rounded-3xl bg-gradient-to-br from-indigo-600 to-indigo-800 text-white shadow-xl shadow-indigo-500/25 flex flex-col justify-between">
          <div>
            ...
            <h2 className="text-xl font-black mb-1">Create New Room</h2>
            <p className="text-xs text-indigo-100/80 mb-6">
              Spin up a fresh room with an instant shareable URL. You become the host with sizing controls.
            </p>
          </div>
          ...
          >
            <span>Start Estimation Session</span>
        ...
        <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl flex flex-col justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mb-6">
              Enter the room code shared by your team or scrum master to join immediately.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-3xl w-full text-left pt-6 border-t border-slate-200/60 dark:border-slate-800/60">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400">
              Sessions live entirely in memory. Zero databases, zero saved cookies.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-xs font-bold text-slate-800 dark:text-slate-200">
              Voice-Enabled Sizing
            </h3>
            <p className="text-[11px] text-slate-500 dark:text-slate-400">
              Speak story titles directly via microphone with interim live text streaming.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400">
              Queue future tickets asynchronously without disrupting current votes.
            </p>
          </div>
        </div>
      </div>
